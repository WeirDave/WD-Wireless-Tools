...
- **NEVER write these files via bash/sed/redirects** — the sandbox mount truncates writes (it broke `walls.html` once). Use the Edit/Write tools only. bash *reads* are also often stale — trust the canonical Read tool.
- Keep version stamps current (Cloud Manager header + About; Quick Walls header; Folder Organizer header + About). Version = David's cache indicator.
