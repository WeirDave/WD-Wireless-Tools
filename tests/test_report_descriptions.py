...
existed. The names did not carry it: "AP Placement Map", "Predictive Design /
AP Placement" and "AP Installation" all put access points on a floor plan.

Two of those three have since become one. Predictive Design differed from the
Placement Map only in whether large floors were split, which is a setting and
not a template, so it was retired into the map. The pair that remains still has
to be told apart, and still is - by audience and page count.
...
class ReportDescriptions(unittest.TestCase):
    ...
    def test_the_two_that_both_draw_aps_on_a_plan_differ_on_what_you_get(self):
        ...
        outputs = {rid: self.reports[rid]["output"]
                   for rid in ("placement", "location")}
        self.assertEqual(len(set(outputs.values())), 2,
                         f"these must not describe the same output: {outputs}")

    def test_the_retired_template_is_gone_and_redirected(self):
        """Retired, not deleted from under anyone: an old selection has to land
        on the survivor rather than on an error."""
        self.assertNotIn("predictive", self.reports)
        src = REPORT_JS.read_text(encoding="utf-8")
        self.assertIn("RETIRED_REPORTS = { predictive: 'placement' }", src)
        self.assertIn("id = RETIRED_REPORTS[id] || id;", src)

    def test_the_survivor_says_the_split_is_configurable(self):
        """It is the setting that replaced a whole template, so it has to be
        visible in the description rather than discovered."""
        d = self.reports["placement"]
        self.assertRegex(d["description"] + " " + d["output"], r"(?i)split")
