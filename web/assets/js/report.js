...
(function () {
  ...
  // Persistent branding logo (data URL). Survives reloads.
  var savedLogo = null;
  ...
  window.pickLogo = function () {
    ...
    picker.onchange = function () {
      ...
      reader.onload = function () {
        ...
        try { localStorage.setItem('wd-report-logo', savedLogo); } catch (e) {
          showToast('Logo saved for this session only (storage full)', 'warn');
        }
        updateLogoPreview();
        renderReport();
      };
  ...
  // First render so the preview shows on load if a logo was persisted.
  updateLogoPreview();
  ...
  function renderAntennaReport(aps, opts) {
    ...
    var head = opts.cover ? renderCover(aps, dateStr) : renderInlineHeader(aps, dateStr);
  ...
  function renderCover(aps, dateStr) {
    var logo = savedLogo
    ...
    return '<section class="rep-cover">'
      + logo
      + '<div class="rep-cover-brand"><span class="rep-brand-icon">&#128203;</span> Report &middot; AP Placement</div>'
  ...
  function renderInlineHeader(aps, dateStr) {
