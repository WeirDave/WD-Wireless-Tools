...
* **A19. `_building_token` breaks on 3-digit building numbers [measured].**
...
* **A22. Pairing is greedy in Ekahau's listing order [reported].** No global
...
* **A23. `exact` is case-sensitive [reported]** despite the docstring
...
* **A24. Stale not-match entries are never pruned [reported].** Nothing removes
...
* **A25. `_ESX_META_CACHE` keys on `(path, int(mtime))` [reported]** — one-second
...
* **A27. A failed background poll wipes the list [reported].** `onData` checks
...
* **A28. The undo path is dead code [reported].** No `opEnqueue` call site
  anywhere supplies `undoFn`; every one passes `undoable: false`.

* **A29. `wd-match-help-seen` is written and never read [measured].**
